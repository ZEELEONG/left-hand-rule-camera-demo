...
const MEDIAPIPE_SOURCES = [
  {
    name: "本地 MediaPipe",
    module: "./vendor/mediapipe/vision_bundle.mjs",
    wasm: "./vendor/mediapipe/wasm",
  },
];

const HAND_MODEL_URL = "./vendor/models/hand_landmarker.task";
...
async function setupHandDetector() {
  ...
  for (const source of MEDIAPIPE_SOURCES) {
    try {
      setStatus(`识别模型加载中：${source.name}`, false);
      ...
      state.detector = await vision.HandLandmarker.createFromOptions(fileset, {
        baseOptions: {
      ...
      console.warn(`MediaPipe full setup failed: ${source.name}`, error);
      failures.push(`${source.name}: ${error.message || error}`);
    }
  }

  throw new Error(`手部识别模型加载失败，请检查网络。${failures.join(" | ")}`);
}
...
async function loadMediaPipeVision() {
  ...
  for (const source of MEDIAPIPE_SOURCES) {
    try {
      ...
      console.warn(`MediaPipe source failed: ${source.name}`, error);
      failures.push(`${source.name}: ${error.message || error}`);
    }
  }

  throw new Error(`手部识别库加载失败，请检查网络或换用 Chrome/Edge。${failures.join(" | ")}`);
}
...
function importWithTimeout(url, timeoutMs) {
  return Promise.race([
    ...
    new Promise((_, reject) => {
      window.setTimeout(() => reject(new Error(`加载超时：${url}`)), timeoutMs);
    }),
  ]);
}
...
function loop(now) {
  ...
  if (video.currentTime !== state.lastVideoTime && state.detector) {
...
function pickLeftHand(result) {
  ...
  for (let i = 0; i < result.landmarks.length; i += 1) {
    ...
    const hand = { landmarks: result.landmarks[i], world, label, score };
